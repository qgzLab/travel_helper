...
*/
	...
	window.onload = function (){
		...
		var yearobj1 =  document.getElementById("year1");
		//当年、月修改时，日期作相应改变
		yearobj1.onchange = changeday1;
		monthobj1.onchange = changeday1;
		}
	 //填充年、月、日,默认时间为当前日期
	function nowday(){
		//1、获得当前日期
		...
		//4、填充日期下拉菜单
		//---1、获得日期下拉菜单对象
		var dateobj = document.getElementById("day");
		//---2、填充日期下拉菜单
	...
	function nowday1(){
		...
		for(var y=nyear;y<=nyear+5;y++){
			op = new Option(y,y);
			yearobj1.options.add(op);			
			}
		//---3、将默认值设置为当前年份
		yearobj1.value=nyear;
		...
		for(var m=1;m<=12;m++){
			op = new Option(m,m);
			monthobj1.options.add(op);			
			}
		//---3、默认值设置为当前月份
		//monthobj.value = nmonth;
		monthobj1.value = nmonth;   
		
		//4、填充日期下拉菜单
		//---1、获得日期下拉菜单对象
		var dateobj1 = document.getElementById("day1");
		//---2、填充日期下拉菜单
		filldate1(nyear,nmonth);
		//---3、将默认值设置为当前日期
		dateobj1.value = ndate;	
		}
		
		//填充日期下拉菜单函数
		function filldate1(year,month){
			//---1、初始化天数计数器，默认值设置为30
			var datecount = 30;
			var op;
			//---2、分别根据年、月重新设置天数
			...
			//3、填充日期下拉菜单
			//获得日期下拉框对象
			var dateobj = document.getElementById("day1");
			for(var d=1;d<=datecount;d++){
				op = new Option(d,d);
				dateobj.options.add(op);
				}				
	...
	function changeday1(){
		//1、分别获得年、月下拉菜单选中值
		var yearvalue1 = document.getElementById("year1").value;
		var monthvalue1 = document.getElementById("month1").value;
		//2、获得日期下拉框对象
		var dateobj1 = document.getElementById("day1");
		//补充！！！：当年月修改时，日期下拉菜单需要重置，需要清空对象
		dateobj1.length = 0;
		//3、调用日期填充函数，填充下拉菜单
		filldate1(yearvalue1,monthvalue1);
		//4、将默认值设为1
		dateobj1.value = 1;
		}